...
function _drawTodoList()
{
    ...
    ProxyState.todoList.forEach(todo => todoListTemplate += todo.Template);
    document.getElementById("todo-list").innerHTML = todoListTemplate;

    const todos = ProxyState.todoList.length;
    const todosDone = ProxyState.todoList.filter(todo => todo.completed).length;
